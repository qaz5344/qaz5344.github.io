...
function play_w(directw) {      //다이렉트 좌우 슬라이드방향
    ...
    $(".rollingbtn").find('li.seq a').each(function(){
        // 슬라이드 이미지아래의 이동 버튼을 초기값으로 동그랑땡으로 만든다.
    ...
    if(wnum == 0) {
        ...
        //슬라이드인덱스가 0번일 때 li 태그를 1초간 서서히 사라지게 한다.
        $(".viewImgList li.imglist2").animate({'opacity':0},1000);
        $(".viewImgList li.imglist0").animate({'opacity':1},1000);
    }else if(wnum == 1) {
        $(".viewImgList li.imglist1").animate({'opacity':1},1000); //단위 1000밀리세컨드
        //슬라이드인덱스가 1번일 때 li 태그를 1초간 서서히 사라지게 한다.
        $(".viewImgList li.imglist2").animate({'opacity':0},1000);
        $(".viewImgList li.imglist0").animate({'opacity':0},1000);
    }else if(wnum == 2) {
        $(".viewImgList li.imglist1").animate({'opacity':0},1000); //단위 1000밀리세컨드
        //슬라이드인덱스가 2번일 때 li 태그를 1초간 서서히 사라지게 한다.
        $(".viewImgList li.imglist2").animate({'opacity':1},1000);
        $(".viewImgList li.imglist0").animate({'opacity':0},1000);
    }
    //true.false 값을 슬라이드 AUTO 변수값으로 사용
    if(wwslidingAuto){clearTimeout(wwslidingAuto); }       //play_w함수 실행을중지한다. 
...
// },3000);
//es7 버전용 (람다식=애로우(화살표)함수사용  .. 코드 생략 간편화)
